# AWS Deployment Guide - Production Profile (2 EC2 Instances + RDS)

## Target Profile

- App tier: **2 × EC2 `t3.large`** (or larger), each running the app stack
- Load balancing: **Application Load Balancer (ALB)** in front of both EC2 instances
- Database tier: RDS PostgreSQL `db.t4g.medium` (or larger)
- Goal: support your 6-condition study with higher availability and lower overload risk
...
## Step 2: Launch 2 EC2 Instances (Production)

### Create Two EC2 Instances
```bash
# Via AWS Console:
1. Go to EC2 → Launch Instance
2. Name: market-mindset-server-1
...
# Repeat and launch a second instance:
# Name: market-mindset-server-2
...
3. Listeners: HTTP 80 (add HTTPS 443 later if using a domain + cert)
4. Availability Zones: select at least 2 AZs in your VPC
5. Target Group: create new target group (type: Instances, protocol: HTTP, port: 80)
6. Health check path: /healthz
7. Register both instances: market-mindset-server-1 and market-mindset-server-2

# Use the ALB DNS name as your public app URL.
```
...
# Connect to instance 2 (repeat all server setup steps there as well)
ssh -i your-key.pem ubuntu@instance-2-public-ip
```
...
scp -i your-key.pem -r /Users/amrita/uchicago/thesis/market-mindset ubuntu@instance-1-public-ip:/home/ubuntu/app
scp -i your-key.pem -r /Users/amrita/uchicago/thesis/market-mindset ubuntu@instance-2-public-ip:/home/ubuntu/app
...
```bash
# Run schema initialization from ONE instance only.
# You can run connection tests from both instances.

# First, test the connection from EC2 to RDS:
psql -h your-rds-endpoint.us-east-1.rds.amazonaws.com -U postgres -d postgres -c "SELECT version();"
# You'll be prompted for the password you set during RDS creation

psql -h your-rds-endpoint.us-east-1.rds.amazonaws.com -U postgres -d postgres -c "CREATE DATABASE \"market-mindset\";"
...
This same service file should be created on both EC2 instances.
...
1. **Access App:** http://your-alb-dns-name (or https://yourdomain.com)
2. **Check Logs:** 
   ```bash
...
# Quick health checks (from each instance)
curl -fsS http://127.0.0.1/healthz
curl -fsS http://127.0.0.1/ >/dev/null && echo "app ok"

# ALB-level check (from your laptop)
curl -fsS http://your-alb-dns-name/healthz
```
...
Exact cost depends on region and traffic. Compared with a single-instance setup, this profile adds the cost of one extra EC2 instance plus ALB charges for higher reliability.
...
scp -i your-key.pem ubuntu@instance-1-public-ip:/home/ubuntu/app/*_export.csv ./study_data/
```
...
scp -i your-key.pem ubuntu@instance-1-public-ip:/home/ubuntu/app/*_export.csv ./study_final_data/
scp -i your-key.pem ubuntu@instance-1-public-ip:/home/ubuntu/final_backup_*.sql ./study_final_data/
